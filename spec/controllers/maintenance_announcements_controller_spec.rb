...
RSpec.describe MaintenanceAnnouncementsController, type: :controller do
    ...
    describe "POST create, successful" do
        before(:each) do
            ...
            @m3 = FactoryBot.create(:machine, owner: @owner1, announcement_deadline: 0)
        end
    
        it "creates a new maintenance announcement affecting a single owner" do
            post :create, params: { maintenance_announcement: @date_params, maintenance_template_id: @template.id, machine_ids: [ @m0.id, @m1.id ] }
        ...
        it "creates a new maintenance announcement affecting multiple owners" do
            post :create, params: {maintenance_announcement: @date_params, impact: "impact", maintenance_template_id: @template.id, machine_ids: [ @m0.id, @m1.id, @m2.id, @m3.id ] }
            expect(MaintenanceAnnouncement.last.begin_date <=> @begin_date).to eq(0)
            expect(MaintenanceAnnouncement.last.end_date <=> @end_date).to eq(0)
    ...
    describe "POST create, unselected VMs and not ignoring" do
        before(:each) do
            @m0 = FactoryBot.create(:machine, owner: @owner0)
            @m1 = FactoryBot.create(:virtual_machine, owner: @owner0, vmhost: @m0.fqdn)
        end
    
        it "renders new" do
            post :create, params: {maintenance_announcement: @date_params, maintenance_template_id: @template.id, machine_ids: [ @m0.id ] }
    ...
    describe "POST create, unselected VMs and ignoring" do
        before(:each) do
            @m0 = FactoryBot.create(:machine, owner: @owner0, announcement_deadline: 0)
            @vm0 = FactoryBot.create(:virtual_machine, owner: @owner0, vmhost: @m0.fqdn, announcement_deadline: 0)
            @vm1 = FactoryBot.create(:virtual_machine, owner: @owner0, vmhost: @m0.fqdn, announcement_deadline: 0)
        end
    ...
    describe "POST create, deadline exceeded" do
        before(:each) do
            ...
            @m0 = FactoryBot.create(:machine, owner: @owner0, announcement_deadline: 7)
        end
    ...
    describe "POST create, one owner has no contact" do
        before(:each) do
            @owner_no_contact = FactoryBot.create(:owner, users: [@current_user])
            @m0 = FactoryBot.create(:machine, owner: @owner0)
            @m1 = FactoryBot.create(:virtual_machine, owner: @owner_no_contact, vmhost: @m0.fqdn)
        end
    ...
    describe "POST create, all owners have contact" do
        before(:each) do
            @m0 = FactoryBot.create(:machine, owner: @owner0, announcement_deadline: 0)
            @m1 = FactoryBot.create(:virtual_machine, owner: @owner1, vmhost: @m0.fqdn, announcement_deadline: 0)
        end
    ...
    describe "check_owner_contacts" do
        it "filters owners with contacts, returning every owner without contact" do
            owner0 = FactoryBot.create(:owner, announcement_contact: "test@example.com")
            owner1 = FactoryBot.create(:owner, announcement_contact: "")
    ...
    describe "deadline_exists" do
        it "filters machines with deadlines, returning machines without deadlines" do
            m0 = FactoryBot.create(:machine, announcement_deadline: 14)
            m1 = FactoryBot.create(:machine, announcement_deadline: nil)
